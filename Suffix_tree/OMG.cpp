...
std::vector<Edge> init_ocurrence_vector(){

    //helps to update the values of the edges using a depth search algorithm
    stack<Edge> stack;
    //helps with the start of the rising values
    vector<Edge> leaves;
    //continuation of the rising values
    vector<Edge> fathers;
    //edges in the suffix tree
    vector<Edge> edges;
    //save the inner nodes for later
...
void period_detection(vector<Edge> inner_edges, string subject, string channel){
    ...
    for(int a=0;a<inner_edges.size();a++){
        for(int j=0;j<inner_edges[a].occur_vec.size()-1;j++){
            ...
            if(conf >= 0.9){
                //let's hope for copy not reference
                //we'll be changing the period object constantly
